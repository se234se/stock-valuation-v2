...
<div class="cards" id="statsCards">
  <div class="card"><div class="label">总访问量</div><div class="value" id="totalVisits">-</div></div>
  <div class="card"><div class="label">独立访客</div><div class="value" id="uniqueIPs">-</div></div>
  <div class="card"><div class="label">今日</div><div class="value" id="todayVisits">-</div></div>
</div>
<div class="grid" id="statsGrid">
  <div class="section">
    <h2>最近访问</h2>
    <table><thead><tr><th>时间</th><th>IP</th><th>地区</th><th>设备</th><th>屏幕</th><th>语言</th></tr></thead>
    <tbody id="recentTable"><tr><td colspan="6" style="text-align:center;color:#94a3b8">加载中...</td></tr></tbody></table>
  </div>
  <div>
    <div class="section"><h2>地区</h2><table><thead><tr><th>地区</th><th>次数</th></tr></thead><tbody id="countryTable"><tr><td colspan="2" style="text-align:center;color:#94a3b8">加载中...</td></tr></tbody></table></div>
    <div class="section"><h2>设备</h2><table><thead><tr><th>设备</th><th>次数</th></tr></thead><tbody id="deviceTable"><tr><td colspan="2" style="text-align:center;color:#94a3b8">加载中...</td></tr></tbody></table></div>
    <div class="section"><h2>日期</h2><table><thead><tr><th>日期</th><th>次数</th></tr></thead><tbody id="dayTable"><tr><td colspan="2" style="text-align:center;color:#94a3b8">加载中...</td></tr></tbody></table></div>
  </div>
</div>
...
async function uploadData(){
  ...
  const upwd = prompt('请输入管理密码');
  if(!upwd) return;
  const res = await fetch('/api/update?pwd='+encodeURIComponent(upwd),{
...
async function loadStats(){
  ...
  document.getElementById('totalVisits').textContent = s.total;
  document.getElementById('uniqueIPs').textContent = s.uniqueIPs;
  document.getElementById('todayVisits').textContent = s.byDay[new Date().toISOString().slice(0,10)] || 0;
  document.getElementById('recentTable').innerHTML = s.recent.map(l => 
    '<tr><td>'+l.timestamp+'</td><td>'+l.ip+'</td><td>'+l.country+'</td><td>'+l.device+'</td><td>'+l.screen+'</td><td>'+l.lang+'</td></tr>'
  ).join('') || '<tr><td colspan="6" style="text-align:center;color:#94a3b8">暂无数据</td></tr>';
  const sortEntries = o => Object.entries(o).sort((a,b)=>b[1]-a[1]).map(([k,v])=>'<tr><td>'+k+'</td><td>'+v+'</td></tr>').join('');
  document.getElementById('countryTable').innerHTML = sortEntries(s.byCountry) || '<tr><td colspan="2" style="text-align:center;color:#94a3b8">暂无数据</td></tr>';
  document.getElementById('deviceTable').innerHTML = sortEntries(s.byDevice) || '<tr><td colspan="2" style="text-align:center;color:#94a3b8">暂无数据</td></tr>';
  document.getElementById('dayTable').innerHTML = Object.entries(s.byDay).sort().map(([d,n])=>'<tr><td>'+d+'</td><td>'+n+'</td></tr>').join('') || '<tr><td colspan="2" style="text-align:center;color:#94a3b8">暂无数据</td></tr>';
}
loadStats();
</script>
</body></html>`;
